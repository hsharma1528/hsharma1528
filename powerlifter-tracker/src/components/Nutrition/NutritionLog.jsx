...
import { Plus, Trash2, ChevronLeft, ChevronRight, Save, X, Apple, Droplets, Flame, Search } from 'lucide-react'
import { useApp } from '../../context/AppContext'
import { getNutritionByDate, saveNutritionLog } from '../../utils/storage'
...
const COMMON_FOODS = [
  ...
  { name: 'Peanut butter (32g / 2 tbsp)', calories: 190, protein: 8, carbs: 6, fat: 16 },
  ...
  { name: 'Milk whole (240ml)', calories: 149, protein: 8, carbs: 12, fat: 8 },
...
function MacroChip({ label, value, color }) {
  return (
    <div className={`flex flex-col items-center px-3 py-2 rounded-xl ${color}`}>
      ...
      <span className="text-white font-bold text-sm">{Math.round(value)}g</span>
    </div>
  )
}
      ...
      <div className="bg-dark-800 border border-dark-700 rounded-2xl w-full max-w-md max-h-[75vh] flex flex-col">
        ...
        {tab === 'manual' && (
          <div className="p-4 space-y-3 overflow-y-auto">
            ...
            <button onClick={handleManual} disabled={!manual.name.trim() || !manual.calories}
...
function MealSection({ meal, onUpdateFoods, onDelete, onChangeName }) {
  ...
  const addFood = (food) => onUpdateFoods([...meal.foods, food])
  const removeFood = (id) => onUpdateFoods(meal.foods.filter((f) => f.id !== id))

  return (
    <div className="bg-dark-800 rounded-2xl border border-dark-700 overflow-hidden">
      {showPicker && <FoodPicker onAdd={addFood} onClose={() => setShowPicker(false)} />}
      ...
      <div className="flex items-center justify-between p-4 border-b border-dark-700">
        ...
        <div className="flex items-center gap-3">
          {totals.calories > 0 && (
            <span className="text-dark-400 text-xs">{Math.round(totals.calories)} kcal</span>
          )}
      ...
      <div className="p-3 space-y-1">
        {meal.foods.map((food) => (
          <div key={food.id} className="flex items-center justify-between bg-dark-900 rounded-xl px-3 py-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => removeFood(food.id)} className="text-dark-500 hover:text-red-400 transition-colors">
        ...
        <button onClick={() => setShowPicker(true)}
          className="flex items-center gap-2 text-dark-400 hover:text-white text-sm py-1.5 pl-2 transition-colors">
...
export default function NutritionLog() {
  ...
  useEffect(() => {
    const existing = getNutritionByDate(currentUser.id, selectedDate)
    if (existing) {
      setLog(existing)
      setWater(existing.water || 0)
    } else {
      setLog({ date: selectedDate, meals: [], water: 0 })
      setWater(0)
    }
  }, [selectedDate, currentUser.id])

  const save = (updatedLog) => {
    const toSave = { ...updatedLog, water, updatedAt: new Date().toISOString() }
    saveNutritionLog(currentUser.id, toSave)
    setLog(toSave)
  }
  ...
  const saveWater = (val) => {
    ...
    if (log) {
      ...
      saveNutritionLog(currentUser.id, updated)
      setLog(updated)
    }
  }
  ...
  const addMeal = () => {
    const updated = {
      ...
      meals: [
        ...(log.meals || []),
        { id: uuidv4(), name: MEAL_TYPES[Math.min(log.meals?.length || 0, MEAL_TYPES.length - 1)], foods: [] },
      ],
    }
    save(updated)
  }

  const updateMealFoods = (mealId, foods) => {
    const updated = { ...log, meals: log.meals.map((m) => (m.id === mealId ? { ...m, foods } : m)) }
    save(updated)
  }

  const updateMealName = (mealId, name) => {
    const updated = { ...log, meals: log.meals.map((m) => (m.id === mealId ? { ...m, name } : m)) }
    save(updated)
  }

  const deleteMeal = (mealId) => {
    const updated = { ...log, meals: log.meals.filter((m) => m.id !== mealId) }
    save(updated)
  }
  ...
  const targets = {
    calories: currentUser.targetCalories || 2500,
    protein: currentUser.targetProtein || 180,
    carbs: currentUser.targetCarbs || 300,
    fat: currentUser.targetFat || 80,
  }
  ...
  return (
    <div className="p-4 lg:p-8 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between bg-dark-800 border border-dark-700 rounded-2xl p-3 mb-6">
        <button onClick={prevDay} className="text-dark-400 hover:text-white transition-colors p-2 rounded-xl hover:bg-dark-700">
        ...
        <button onClick={nextDay} disabled={isToday}
          className="text-dark-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors p-2 rounded-xl hover:bg-dark-700 disabled:hover:bg-transparent">
      ...
      <div className="bg-dark-800 border border-dark-700 rounded-2xl p-5 mb-6">
        ...
        <div className="h-3 bg-dark-700 rounded-full overflow-hidden mb-4">
          <div className={`h-full rounded-full transition-all duration-500 ${
            calPct >= 100 ? 'bg-orange-500' : 'bg-brand-500'
          }`} style={{ width: `${calPct}%` }} />
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Protein', val: totals.protein, target: targets.protein, color: 'bg-dark-700' },
            { label: 'Carbs', val: totals.carbs, target: targets.carbs, color: 'bg-dark-700' },
            { label: 'Fat', val: totals.fat, target: targets.fat, color: 'bg-dark-700' },
              ...
              <div key={label} className={`${color} rounded-xl p-3`}>
                ...
                <div className="h-1.5 bg-dark-600 rounded-full mt-2 overflow-hidden">
                  <div className={`h-full rounded-full ${
                    label === 'Protein' ? 'bg-brand-500' : label === 'Carbs' ? 'bg-yellow-500' : 'bg-blue-500'
                  }`} style={{ width: `${pct}%` }} />
                </div>
              </div>
            )
        ...
        <div className="flex items-center gap-3 mt-4 pt-4 border-t border-dark-700">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => saveWater(Math.max(0, water - 8))}
              className="w-7 h-7 rounded-lg bg-dark-700 hover:bg-dark-600 text-white transition-colors flex items-center justify-center text-sm font-bold">−</button>
            <span className="text-white font-semibold w-10 text-center">{water}</span>
            <button onClick={() => saveWater(water + 8)}
              className="w-7 h-7 rounded-lg bg-dark-700 hover:bg-dark-600 text-white transition-colors flex items-center justify-center text-sm font-bold">+</button>
          </div>
        </div>
      </div>
